...
def count_averarage_stability_and_cycle_amount(automata_list):
    ...
    stability_standard_error = math.sqrt(
        float(stability_square_sum) / automata_list_length
    )
    cycle_amount_standard_error = math.sqrt(
        float(cycle_amount_square_sum) / automata_list_length
    )

    print(
        "Average stability:",
        str(average_stability) + "+-" + str(stability_standard_error * 3),
    )
    print(
        "Average cycle_amount:",
        str(average_cycle_amount) + "+-" + str(cycle_amount_standard_error * 3),
    )
...
def generate_stat_file_name(N, K, I):
    ...
    return (
        ...
        + "__i_"
        + add_succeeding_zeroes(max_i, I)
    )


def create_automata_stability_samples(
    N, K, current_folder_path, sample_length, samples_amount
):
    automata_list = []
    print("gather_data:")
...
def create_automata_return_time_samples(
    N, K, current_folder_path, sample_length, samples_amount
):
    automata_list = []
    print("gather_data:")
