...
class NK_Automata(object):
    ...
    def __init__(
        ...
        self.state_span = (
            {}
        )  # state_span: {current_state_number: next_state_number,...}
        self.state_list = []
        self.attractor_dict = (
            {}
        )  # attractor_dict {attractor_number:[size,basin_size],...}
        self.attractor_states_dict = (
            {}
        )  # attractor_states_dict: {attractor_state_number:[next_attractor_state_number,attractor_state_weight],...}
    ...
    def __sizeof__(self):
        return (
            ...
            + sys.getsizeof(self.links_list)
            + sys.getsizeof(self.state_span)
            + sys.getsizeof(self.basin_amount)
            + sys.getsizeof(self.stability)
    ...
    def generate_random_automata(self):
        self.functions_list = []
        self.links_list = []
        generate_automata.generate_n_k_automata(
            self.N, self.K, self.functions_list, self.links_list
        )
    ...
    def step_automata(self, state):
        ...
        for bool_fun_number in range(self.N):
            ...
            for state_element_number in range(self.K):
                bool_fun_inputs += state.as_string()[
                    self.links_list[bool_fun_number][state_element_number]
                ]

            new_state_string += self.functions_list[bool_fun_number].evaluate(
                bool_fun_inputs
            )
    ...
    def process_sample(self, seed):
        ...
        while not (current_state_number in sample_list):
            if self.state_list[current_state_number].in_attractor:
                for sample_state_number in sample_list:
                    ...
                    self.state_list[sample_state_number].basin_number = self.state_list[
                        current_state_number
                    ].basin_number
                    self.state_list[
                        sample_state_number
                    ].first_attractor_state_number = current_state_number
                self.state_list[current_state_number].weight += len(sample_list)
                return

            if self.state_list[current_state_number].in_basin:
                first_attractor_state_number = self.state_list[
                    current_state_number
                ].first_attractor_state_number
                for sample_state_number in sample_list:
                    self.state_list[sample_state_number].in_basin = True
                    self.state_list[sample_state_number].basin_number = self.state_list[
                        current_state_number
                    ...
                    self.state_list[
                        sample_state_number
                    ].first_attractor_state_number = first_attractor_state_number
                self.state_list[first_attractor_state_number].weight += len(sample_list)
                return

            sample_list.append(current_state_number)
            current_state_number = (
                self.step_automata(State(current_state_number, self.N))
            ).as_int()
        ...
        for state_number in basin_list:
            ...
            self.state_list[
                state_number
            ].first_attractor_state_number = attractor_start_state_number
    ...
    def analyse_automata(self):
        ...
        for state_number in range(2**self.N):
            if (
                self.state_list[state_number].in_basin == False
                and self.state_list[state_number].in_attractor == False
            ):
    ...
    def initialize_attractor_dict(self):
        for attractor_number in range(1, self.basin_amount + 1):
            self.attractor_dict[attractor_number] = [
                0,
                0,
            ]  # {attractor_number:[size,basin_size],...}
    ...
    def count_stability(self):
        ...
        for attractor_number in self.attractor_dict:
            basin_size_square_sum += (
                self.attractor_dict[attractor_number][0]
                + self.attractor_dict[attractor_number][1]
            ) ** 2
    ...
    def count_expected_return_time(self):
        ...
        for current_state in self.state_list:
            if current_state.in_basin == True:
                ...
                first_attractor_state = self.state_list[
                    current_state.first_attractor_state_number
                ]

                average_return_time += self.distance_between_states(
                    current_state, first_attractor_state
                )
    ...
    def make_attractor_states_dictionary(self):
        ...
        for state in self.state_list:
            if state.in_attractor == True:
                if self.view_states_as_binary:
                    self.attractor_states_dict[state.as_string()] = [
                        (self.step_automata(state)).as_string(),
                        state.weight,
                    ]
                else:
                    self.attractor_states_dict[state.as_int()] = [
                        (self.step_automata(state)).as_int(),
                        state.weight,
                    ]
