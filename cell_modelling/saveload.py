...
def generate_automata_type_folder_name(N, K):
    ...
    return (
        "N_"
        + add_succeeding_zeroes(max_n_k, N)
        + "__k_"
        + add_succeeding_zeroes(max_n_k, K)
    )
...
def generate_automata_folder_name(automata_type_folders_collection):
    ...
    return add_succeeding_zeroes(
        max_automata_folders_amount, current_automat_folders_amount
    )
...
def save_n_k_automata(current_folder_path, nk_automata, picture=False):
    ...
    automata_type_folder_name = generate_automata_type_folder_name(
        nk_automata.N, nk_automata.K
    )

    automata_type_folder_path = (
        automata_types_folder_path + "/" + automata_type_folder_name
    )
    ...
    if picture:
        graph_file_name = (
            automata_type_folder_name + "_" + automata_folder_name + ".png"
        )
        graph_cycles_only_file_name = (
            automata_type_folder_name
            + "_"
            + automata_folder_name
            + "__cycles_only"
            + ".png"
        )
        graph_fun_links_file_name = (
            automata_type_folder_name
            + "_"
            + automata_folder_name
            + "__fun_links"
        ...
        if os.path.exists(current_folder_path + "/" + "temp_pic.png"):
            shutil.copyfile(
                current_folder_path + "/" + "temp_pic.png",
                os.path.join(data_folder_path, graph_file_name),
            )
        if os.path.exists(data_folder_path + "/" + "temp_pic2.png"):
            shutil.copyfile(
                data_folder_path + "/" + "temp_pic2.png",
                os.path.join(automata_folder_path, graph_cycles_only_file_name),
            )
        if os.path.exists(data_folder_path + "/" + "temp_pic3.png"):
            shutil.copyfile(
                data_folder_path + "/" + "temp_pic3.png",
                os.path.join(automata_folder_path, graph_fun_links_file_name),
            )
...
def gather_data(automata_list, automata_folders_folder_path):
    ...
    for automata_folder_name in automata_folders_list:
        print("gathering:", i)
        i += 1
        nk_automata = load_variable_from_file(
            "automata.txt", automata_folders_folder_path + "/" + automata_folder_name
        )
...
def test_folders_creation(data_folder_path):
    ...
    for n in range(N):
        for k in range(K):
            for i in range(I):
                save_n_k_automata(
                    automata, enters, initial_state, data_folder_path, n, k
                )
